      ...
      try {
        ...
        await client.query(`
          INSERT INTO settings (
            id, shop_latitude, shop_longitude, shop_whatsapp_number, minimum_order_amount, preparation_time_base, delivery_zones_json, promo_codes_json, hero_images_json, is_store_open, closed_message, updated_at
          )
          VALUES (1, $1, $2, $3, $4, $5, $6::jsonb, $7::jsonb, $8::jsonb, $9, $10, NOW())
    ...
    try {
      ...
      const orderResult = await client.query(`
        INSERT INTO orders (
          ...
          mode, address, latitude, longitude, distance_km, delivery_zone_radius,
      ...
      return { order: mapOrderRow(savedOrder), customer: mapCustomerRow(customer) };
    } catch (error) {
...
async function listOrders(filters = {}) {
  return withDatabase(async () => {
    ...
    return result.rows.map(row => ({
      ...mapOrderRow(row),
      estimatedTime: getEstimatedTime(row, activeOrders, settings.preparationTimeBase)
    }));
  });
}
...
async function getOrderById(orderId) {
  return withDatabase(async () => {
    ...
    return {
      ...mapOrderRow(row),
      estimatedTime: getEstimatedTime(row, Number(activeOrdersResult.rows[0]?.active_count || 0), settings.preparationTimeBase),
...
async function updateOrderStatus(orderId, status) {
  return withDatabase(async () => {
    ...
    return result.rows[0]
      ? { ...mapOrderRow(result.rows[0]), estimatedTime: getEstimatedTime(result.rows[0], Number(activeOrdersResult.rows[0]?.active_count || 0), settings.preparationTimeBase) }
      : null;
  });
}
