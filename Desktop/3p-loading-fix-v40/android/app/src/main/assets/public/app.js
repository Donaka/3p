...
function updateStoreStatusUi() {
  ...
  if (checkoutButton) checkoutButton.disabled = closed || getCartDetails().total === 0;
}
...
function makeTimeline(statusLabel) {
  const steps = ["Nouvelle", "Acceptee", "En preparation", "Livree"];
  const normalized = String(statusLabel || "").toLowerCase();
  const currentIndex = normalized.includes("annule") ? -1 : Math.max(0, steps.findIndex(step => step.toLowerCase() === normalized));
  const progressPercent = currentIndex < 0 ? 0 : Math.min(100, (currentIndex / (steps.length - 1)) * 100);
...
function renderOrderHistory() {
  ...
  const orders = getDisplayedOrderHistory();
  ordersList.innerHTML = orders.length ? orders.map(order => {
    const timeline = makeTimeline(order.status);
    return `
      <article class="order-history-card" data-order-card="${order.id}" tabindex="0" role="button" aria-label="Voir les details de la commande ${order.id}">
        <div class="order-history-top">
          <div>
            <p>${order.dateLabel}</p>
            <h3>${order.mode === "delivery" ? "Livraison" : "Pickup"} Â· ${order.total}</h3>
            ${order.estimatedTime ? `<small class="order-estimate">Livraison estimee: ${order.estimatedTime}</small>` : ""}
          </div>
          <span class="order-status-pill">${order.status}</span>
        </div>
        <div class="order-timeline">
          ...
          <div class="order-timeline-steps">
            ${timeline.steps.map((step, index) => `<span class="timeline-step ${timeline.currentIndex >= index ? "active" : ""} ${timeline.currentIndex === index ? "current" : ""}">${step}</span>`).join("")}
          </div>
        </div>
        <div class="order-history-meta">
          <small>${order.itemCount} items</small>
...
function renderOrderDetails(order) {
  ...
  const timeline = makeTimeline(localizeOrderStatus(order.rawStatus || order.status));
...
async function initFirebaseAuth() {
  ...
  if (isNativeAndroidApp() && nativeAuth) {
    ...
    try {
      ...
      console.warn("Native Firebase auth state unavailable", error);
    }
    return;
  }
  ...
  try {
    ...
    await getRedirectResult(firebaseAuth).catch(error => {
      console.warn("Firebase redirect login failed", error);
    ...
    console.warn("Firebase could not start", error);
    accountHelp.textContent = "Google login could not load. Check Firebase config.";
  }
}
...
function renderProducts() {
  ...
  if (homeResultsMeta) homeResultsMeta.textContent = `${featured.length} featured picks`;
...
function getCartDetails() {
  ...
  return { items, subtotal, fee, distanceKm, matchedZone, promoDiscount, total: beforeDiscount - promoDiscount };
}

function renderCart() {
  const { items, subtotal, fee, distanceKm, matchedZone, promoDiscount, total } = getCartDetails();
  ...
  const checkoutButton = document.querySelector("#checkoutButton");
  if (checkoutButton) checkoutButton.disabled = isStoreClosed() || total === 0;
...
orderDetailsWhatsapp?.addEventListener("click", () => {
  if (!selectedOrderDetails) return;
  openExternalUrl(`https://wa.me/212688943959?text=${encodeURIComponent(`Bonjour, j'ai une question sur la commande #${selectedOrderDetails.id}.`)}`);
});
