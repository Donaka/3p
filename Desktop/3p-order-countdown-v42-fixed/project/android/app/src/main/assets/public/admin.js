...
function renderCategoryList() {
  ...
  const categories = menu.categories.filter(category => category.name.toLowerCase().includes(query));
...
categoryForm.addEventListener("submit", async event => {
  ...
  if (oldName) {
    menu.categories = menu.categories.map(category =>
      category.name === oldName ? { ...category, name: nextName, imageUrl: nextImageUrl } : category
    );
  ...
  menu.categories.push({ name: nextName, imageUrl: nextImageUrl });
  resetCategoryForm();
